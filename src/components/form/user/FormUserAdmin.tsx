...
const FormUserAdmin: React.FC = () => {
  ...
  const onSubmit = async (data: FormUserData) => {
    ...
    try {
      ...
      await dispatch(getUsers({ page: currentPage, size: pageSize }));
    } catch (error: unknown) {
  ...
  const handleMinTotalSpentChange = (
  ...
  const filteredUsers = users.filter((user) => {
    ...
    if (dateFrom) {
      matchesDate =
        matchesDate && new Date(user.dateJoined) >= new Date(dateFrom);
    }
    if (dateTo) {
      matchesDate =
        matchesDate && new Date(user.dateJoined) <= new Date(dateTo);
    }
  ...
  [...users]
    ...
    .slice(0, 5)
    .forEach((u) =>
      topSpenders.push({ username: u.username, totalSpent: u.totalSpent || 0 })
    );
  ...
  const averageMonthlyJoins =
    numberOfMonths > 0 ? totalElements / numberOfMonths : 0;
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 text-center">
          ...
          <div>
            <p className="text-xl font-bold">
              {averageSpending.toLocaleString(undefined, {
          ...
          <div>
            <p className="text-xl font-bold">
              {averageMonthlyJoins.toLocaleString(undefined, {
                maximumFractionDigits: 2,
              })}
            </p>
