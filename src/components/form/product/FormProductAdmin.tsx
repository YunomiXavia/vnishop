...
import TextAreaProductForm from "@/components/textArea/TextAreaProductForm";
...
const FormProductAdmin: React.FC = () => {
  ...
  const [subscriptionDurationFilter, setSubscriptionDurationFilter] = useState<
    number | null
  >(null);
    ...
    try {
      ...
      if (imageFile) {
        if (
          ...
          setNotification({
            message: "Chỉ cho phép định dạng jpg/jpeg/png",
            type: "error",
          });
          return;
        }
      ...
      formData.append(
        ...
        data.subscriptionDuration.toString()
      );
  ...
  const onUpdateSubmit = async (data: FormUpdateProductData) => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        updateProduct({ id: currentProductId, formData })
      );
      if (updateProduct.fulfilled.match(resultAction)) {
        ...
        updateProduct.rejected.match(resultAction) &&
        resultAction.payload
      ) {
        const error = resultAction.payload;
        setNotification({
  ...
  const handleDeleteProducts = async (productIds: string[]) => {
    try {
      ...
      if (deleteProducts.fulfilled.match(resultAction)) {
        ...
        deleteProducts.rejected.match(resultAction) &&
        resultAction.payload
      ) {
        const error = resultAction.payload;
        setNotification({
  ...
  const handleExportStatisticsToExcel = () => {
    try {
      ...
      const averageSubscriptionDuration =
        products.length > 0
          ? products.reduce(
              (sum, product) => sum + product.subscriptionDuration,
              0
            ) / totalProducts
          : 0;
      ...
      const overviewData = [
        { ["Chỉ số"]: "Tổng SP", ["Giá trị"]: totalProducts },
        {
          ...
          ["Giá trị"]: totalValue.toLocaleString(),
        },
      ...
      const categoryData = Object.keys(categoryDistribution).map(
        (category) => ({
          ["Danh mục"]: category,
      ...
      const topSubscriptionProductsData = topSubscriptionProducts.map(
        (product) => ({
          ["Tên"]: product.productName,
          ["Thời lượng"]: product.subscriptionDuration,
        })
      ...
      const topSubscriptionWorksheet = XLSX.utils.json_to_sheet(
        topSubscriptionProductsData
      );
      XLSX.utils.book_append_sheet(
        workbook,
        topSubscriptionWorksheet,
        "Top Đăng ký"
      );
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (
      subscriptionDurationFilter !== null &&
      subscriptionDurationFilter !== undefined &&
      !Number.isNaN(subscriptionDurationFilter)
    ) {
      condition =
        condition && product.subscriptionDuration > subscriptionDurationFilter;
    }
    return condition;
  });
  ...
  if (productError)
    return (
      <p>
        Lỗi {productError.code}: {productError.message}
      </p>
    );
  ...
  const topSubscriptionData = {
    ...
    datasets: [
      {
        label: "Thời lượng",
        data: topSubscriptionProducts.map(
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <TextAreaProductForm
                    ...
                    validation={{
                      minLength: {
                        value: 10,
                        message: "Mô tả phải dài ít nhất 10 ký tự",
                      },
                    }}
                  />
                  <InputTextProductForm
            ...
            <tbody>
              {filteredProducts.length > 0 ? (
                filteredProducts.map((product, index) => {
                  const tinyImageName = product.originalImageName
                    ? product.originalImageName.replace("original_", "tiny_")
                    : "";
                  const tinyImageUrl = tinyImageName
                    ? `${BASE_URL}/products/${product.productCode}/tiny/${tinyImageName}`
                    : "";
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="border px-4 py-2 text-center">
                        {product.subscriptionDuration} ngày
                      </td>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <Pagination
          ...
          onPageChange={(newPage) =>
            dispatch(getProducts({ page: newPage - 1, size: pageSize }))
          }
        />
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 text-center">
          ...
          <div>
            <p className="text-xl font-bold">
              {averageSubscriptionDuration.toFixed(2)}
            </p>
            <p>Thời lượng TB</p>
          </div>
        </div>
