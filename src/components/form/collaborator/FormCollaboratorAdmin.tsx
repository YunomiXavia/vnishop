...
const FormCollaboratorAdmin: React.FC = () => {
  ...
  const handleDeleteCollaborator = async (userId: string) => {
    try {
      ...
      // Reload collaborators after deletion
  ...
  const onSubmit = async (data: FormCollaboratorData) => {
    try {
      ...
      // Reload collaborators after creation/update
      await dispatch(getCollaborators({ page: currentPage, size: pageSize }));
    } catch (error: unknown) {
      const err = extractError(error);
      setNotification({
  ...
  // Handle Row Select for Bulk Delete
  const handleBulkSelect = (userId: string) => {
    setSelectedCollaborators((prevCollaborators) => {
      if (prevCollaborators.includes(userId)) {
        return prevCollaborators.filter((id) => id !== userId);
      }
      return [...prevCollaborators, userId];
    });
  };

  // Handle Row Select for Showing Orders
  const handleRowSelect = (collaboratorId: string) => {
    if (selectedCollaboratorId === collaboratorId) {
  ...
  const filteredCollaborators = collaborators.filter((col) => {
    if (
      ...
      col.commissionRate < commissionRateFilter
    ) {
      return false;
    }
    if (
      totalOrdersFilter !== null &&
      (col.totalOrdersHandled || 0) < totalOrdersFilter
    ) {
      return false;
    }
    if (
      commissionEarnedFilter !== null &&
      (col.totalCommissionEarned || 0) < commissionEarnedFilter
    ) {
      return false;
    }
    return !(
      surveyHandledFilter !== null &&
      (col.totalSurveyHandled || 0) < surveyHandledFilter
    );
  });
  ...
  const ordersHandledChartData = {
    labels: filteredCollaborators.map(
      (collaborator) => collaborator.user.username
    ),
    datasets: [
      {
        label: "Tổng đơn đã xử lý",
        data: filteredCollaborators.map(
    ...
    datasets: [
      {
        ...
        data: commissionRateDistribution,
  ...
  const handleExportStatisticsToExcel = () => {
    try {
      ...
      const statisticsData = filteredCollaborators.map(
        (collaborator, index) => ({
          STT: index + 1,
          ["Tên đăng nhập"]: collaborator.user.username,
    ...
    <div className="p-4">
      ...
      <div className="p-4 mx-auto ">
          ...
          <div className="flex space-x-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                setShowForm(true);
                setShowFormCommission(false);
                setSelectedCollaboratorId(null); // Bỏ chọn CTV hiện tại khi thêm mới
              }}
            >
        ...
        <div className="flex gap-4 mb-4 justify-end items-center">
          <div>
            <label className="block text-gray-700">HH (≥):</label>
            <input
              type="number"
              step="0.01"
          ...
          <div>
            <label className="block text-gray-700">Đơn (≥):</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-gray-700">HH(VND)(≥):</label>
            <input
              type="number"
                  ...
                  <InputTextForm
                    ...
                    validation={{
                      validate: (value: Date | null) => {
                        ...
                        return value <= today || "Không vượt quá hôm nay";
                      },
                    }}
                  />
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50">
            <FormProvider {...methods}>
              <form
                ...
                ref={formRef}
              >
                <h3 className="text-lg font-semibold mb-4">Sửa HH</h3>
                <div className="grid grid-cols-2 gap-4">
                  {/* HH */}
                  <InputTextForm
                    name="commissionRate"
              ...
              filteredCollaborators.map((collaborator, index) => (
                <React.Fragment key={collaborator.id}>
                  <tr
                    ...
                    onClick={() => handleRowSelect(collaborator.id)}
                  >
                  ...
                  {/* Row để chèn FormOrder nếu CTV được chọn */}
                  {selectedCollaboratorId === collaborator.id && (
                    <tr>
                      <td colSpan={14} className="bg-gray-100">
                        <FormOrder collaboratorId={collaborator.id} />
                      </td>
                    </tr>
                  )}
                </React.Fragment>
      ...
      <div className="col-span-full bg-white p-6 rounded-lg shadow-lg mb-6 mt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 text-center gap-4">
          <div>
            <p className="text-xl font-bold">{formatCurrencyVND(totalSpent)}</p>
            <p>Chi tiêu</p>
          </div>
          <div>
