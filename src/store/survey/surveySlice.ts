  ...
  async ({ page, size }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get(
        `/collaborator/surveys?page=${page}&size=${size}`
      );
...
// Get All Surveys
export const getSurveysForCollaborator = createAsyncThunk<
  {
    content: Survey[];
    currentPage: number;
    totalPages: number;
  ...
  { rejectValue: ErrorResponseProps }
>(
  "/collaborator/surveys/getSurveys-collaborator",
  async ({ page, size }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get(
        `/collaborator/surveys?page=${page}&size=${size}`
...
export const handleSurvey = createAsyncThunk<
  Survey,
  { collaboratorId: string; surveyId: string },
  { rejectValue: ErrorResponseProps }
>(
  ...
  async ({ collaboratorId, surveyId }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.put(
        `/collaborator/survey/${collaboratorId}/${surveyId}`
      );
      if (response.data.code === 2000) {
...
// Handle Surveys
export const handleSurveys = createAsyncThunk<
  Survey[],
  { collaboratorId: string; surveyIds: string[] },
  { rejectValue: ErrorResponseProps }
>(
  ...
  async ({ collaboratorId, surveyIds }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.put(
        `/collaborator/survey/${collaboratorId}`,
        { surveyIds }
      );
      if (response.data.code === 2000) {
        return response.data.result as Survey[];
...
// Response Survey
export const responseSurvey = createAsyncThunk<
  Survey,
  { surveyId: string; collaboratorId: string; responseText: string },
  { rejectValue: ErrorResponseProps }
>(
  ...
  async ({ surveyId, collaboratorId, responseText }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(
        `/collaborator/survey/${surveyId}/${collaboratorId}`,
        {
...
>(
  ...
  async ({ surveyId }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.put(
        `/collaborator/survey/${surveyId}/complete`
      );
      if (response.data.code === 2000) {
        return response.data.result as Survey;
      } else {
    ...
    builder
      ...
      .addCase(getSurveys.rejected, (state, action) => {
        ...
        state.error = action.payload || {
          code: -1,
          message: "Lấy danh sách các câu hỏi thất bại.",
        };
      })
      // Get All Surveys For Colalborator
      .addCase(getSurveysForCollaborator.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getSurveysForCollaborator.fulfilled, (state, action) => {
        state.loading = false;
        state.surveys = action.payload.content;
        state.currentPage = action.payload.currentPage;
        state.totalPages = action.payload.totalPages;
      ...
      .addCase(getSurveysForCollaborator.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || {
          code: -1,
          message: "Lấy danh sách các câu hỏi thất bại.",
      ...
      .addCase(handleSurvey.rejected, (state, action) => {
        ...
        state.error = action.payload || {
          code: -1,
          message: "Xử lý câu hỏi người dùng thất bại.",
        };
      })
      // Handle Surveys
      ...
      .addCase(handleSurveys.fulfilled, (state, action) => {
        ...
        state.surveys = state.surveys.map(
          (survey) =>
            updatedSurveys.find(
              (updated: Survey) => updated.id === survey.id
            ) || survey
        );
      })
      .addCase(handleSurveys.rejected, (state, action) => {
        ...
        state.error = action.payload || {
          code: -1,
          message: "Xử lý các câu hỏi người dùng thất bại.",
        };
      })
      // Response Survey
      ...
      .addCase(responseSurvey.rejected, (state, action) => {
        ...
        state.error = action.payload || {
          code: -1,
          message: "Phản hồi câu hỏi của người dùng thất bại.",
        };
      })
      // Complete Survey
      ...
      .addCase(completeSurvey.rejected, (state, action) => {
        ...
        state.error = action.payload || {
          code: -1,
          message: "Hoàn thành câu hỏi của người dùng thất bại.",
        };
      });
  },
});
